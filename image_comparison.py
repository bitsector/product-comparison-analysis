...
class ImageProductMatcher:
    ...
    def __init__(self):
        ...
        self.competitor_images_dir = "competitor_images/"
...
Compare these two product images and determine if they represent the same or functionally equivalent products for building/construction applications.
...
FOR FAUCETS - Focus on: Handle type (single/double), spout style (gooseneck/straight), spray features, mounting configuration
FOR TOILETS - Focus on: One-piece vs two-piece, bowl shape (elongated/round), flush mechanism, mounting type  
FOR TILES - Focus on: Size, pattern, texture, material type (ceramic/porcelain/natural stone), edge treatment
...
- **Interchangeability**: Can these products serve the same function in a construction project? (YES/NO with explanation)

Focus on functional compatibility for building contractors and architects making product substitutions.
"""

    def compare_product_images(self, competitor_image_path: str, our_image_path: str) -> Dict:
        """Compare two product images using OpenAI Vision model"""
        try:
            # Log without revealing filenames to avoid hints
            logger.info(f"Comparing competitor image with catalog image")
            
            competitor_b64 = self.encode_image(competitor_image_path)
            our_b64 = self.encode_image(our_image_path)
            
            if not competitor_b64 or not our_b64:
                ...
                messages=[
                    {
                        ...
                        "content": [
                            ...
                            {
                                "type": "image_url",
                                "image_url": {"url": f"data:image/jpeg;base64,{competitor_b64}"}
                            },
                            {
                                "type": "text",
    ...
    def find_best_image_matches(self, competitor_image_path: str) -> List[Dict]:
        """Find best matching images from our catalog for a competitor image"""
        our_images = self.get_image_files(self.our_images_dir)
        matches = []
        
        competitor_name = os.path.basename(competitor_image_path)
        logger.info(f"Finding matches for competitor image: {competitor_name}")
        
        for our_image_path in our_images:
            our_name = os.path.basename(our_image_path)
            logger.info(f"  Comparing with our image: {our_name}")
            
            comparison = self.compare_product_images(competitor_image_path, our_image_path)
            ...
            match_info = {
                "competitor_image": competitor_name,
                "our_image": our_name,
            ...
            logger.info(f"    {competitor_name} ←→ {our_name}")
    ...
    def process_all_competitor_images(self) -> List[Dict]:
        """Process all competitor images and find matches"""
        competitor_images = self.get_image_files(self.competitor_images_dir)
        
        if not competitor_images:
            raise ValueError(f"No images found in {self.competitor_images_dir}")
        
        logger.info(f"Found {len(competitor_images)} competitor images to process")
        
        self.results = []
        
        for competitor_image_path in competitor_images:
            matches = self.find_best_image_matches(competitor_image_path)
            
            # Get the best match
            best_match = matches[0] if matches else None
            
            result = {
                "competitor_image": os.path.basename(competitor_image_path),
            ...
            logger.info(f"Best match for {result['competitor_image']}: {result['best_match_image']} (score: {result['similarity_score']:.3f})")
    ...
    def export_results(self, output_file: str = "output/image_matching_results.csv") -> str:
        ...
        try:
            ...
            for result in self.results:
                csv_row = {
                    "competitor_image": result["competitor_image"],
            ...
            logger.info(f"- Total competitor images processed: {total_images}")
...
def main():
    ...
    try:
        ...
        if not os.path.exists(matcher.competitor_images_dir):
            ...
            logger.warning(f"Created {matcher.competitor_images_dir} - please add competitor product images")
        ...
        # Process all images
        results = matcher.process_all_competitor_images()
